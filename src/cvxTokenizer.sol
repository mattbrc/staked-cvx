...
contract cvxTokenizer is ERC4626, Ownable {
    ICVXStakingContract cvxStaking =
    ...
    constructor() ERC4626(CVX, "Golden Ratio Vault", "grVault") {}

    /**
     *Total CVX controlled by the contract is composed of:
     *Golden Ratio totalAssets = pending rewards + staked CVX + CRV Pool ETH +...
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        return (cvxStaking.balanceOf(address(this)) +
    ...
    function beforeWithdraw(uint256 assets, uint256 shares) internal override {
        ...
        route[2] = WETH;
        route[3] = address(CVX);
        uint returnAmount = router.swapExactTokensForTokens(
            earnedRew,
            0,
            route,
            address(this),
    ...
    function calcOutstandingRewards() internal view returns (uint) {
        uint pendingRewardPerToken = (cvxStaking.rewardPerToken() -
            cvxStaking.userRewardPerTokenPaid(address(this)));

        uint outstandingRewards = ((cvxStaking.balanceOf(address(this)) *
            pendingRewardPerToken) / 1e18);
